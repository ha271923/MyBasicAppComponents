...
public class MysynchronizedCollection implements MultiThreadAccessIF {
    ...
    public void ThreadsAccessData(Collection<String> collect, int num) {
        ...
        for (int i=0; i<num; i++) {
            executorService.execute(getReadRunnable(collect));
            executorService.execute(getWriteRunnable(collect, i));
        }
    ...
    public Runnable getReadRunnable(Object obj){
        return new ReadTask((Collection<String>)obj);
    }

    @Override
    public Runnable getWriteRunnable(Object obj, int i){
        return new WriteTask((Collection<String>)obj, i);
    }
